...
5. **"Our repo / our project" means local first** — When user refers to "the day planner repo" or "our project", check the local git repo under `~/.hermes/skills/productivity/adhd-day-planner/` before searching GitHub. The user explicitly called this out as frustrating when missed — treat this as a hard rule, not a suggestion.
6. **`daily/` folder name is not arbitrary** — It aligns with Obsidian CLI's `obsidian tasks daily` command, which only looks in today's daily note. Using a separate `to-do/` folder breaks CLI-based task queries. Do not change the folder name without explicit user direction.
7. **Do NOT modify Obsidian vault config files (`.obsidian/`)** as part of this skill. If asked about daily note auto-generation, explain the relevant setting but do not patch `app.json` or `daily-notes.json` without explicit user approval.
